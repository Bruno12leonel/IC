...
class HDBStream(base.Clusterer):
    ...
    def __init__(
        ...
        self.df_mc_to_points     = df_mc_to_p
        ...
        self.df_runtime = pd.DataFrame(columns=['timestamp', 'data_bubbles', 'mrg', 'mst', 'dendrogram', 'selection', 'total'])
    ...
    def _get_closest_cluster_key(self, point, clusters):
        ...
        for k, cluster in clusters.items():
            center   = cluster.getCenter(self.timestamp)
    ...
    def _merge(self, point):
        ...
        if len(self.p_micro_clusters) != 0:
            ...
            if updated_pdb.getRadius(self.timestamp) <= self.epsilon:
                ...
                if self.p_micro_clusters[closest_pdb_key].hasCenterChanged(self.m_movementThreshold, self.epsilon, self.timestamp):
                    ...
                    p = self.df_mc_to_points[(self.df_mc_to_points['x'] == point[0]) & (self.df_mc_to_points['y'] == point[1])]
                    self.df_mc_to_points.loc[p.index[0], 'id_mc'] = closest_pdb_key
        ...
        if not merged_status and len(self.o_micro_clusters) != 0:
            ...
            if updated_omc.getRadius(self.timestamp) <= self.epsilon:
                ...
                if updated_omc.getWeight(self.timestamp) > self.mu * self.beta:
                    ...
                    if not list(self.p_micro_clusters.keys()):
                        updated_omc.setID(0)
                        self.p_micro_clusters[0] = updated_omc
                    else:
                        new_key = list(self.p_micro_clusters.keys())[-1] + 1
                    ...
                    p = self.df_mc_to_points[(self.df_mc_to_points['x'] == point[0]) & (self.df_mc_to_points['y'] == point[1])]
                    self.df_mc_to_points.loc[p.index[0], 'id_mc'] = new_key
                    ...
                    p = self.df_mc_to_points[(self.df_mc_to_points['x'] == point[0]) & (self.df_mc_to_points['y'] == point[1])]
                    self.df_mc_to_points.loc[p.index[0], 'id_mc'] = (-1) * closest_omc_key
                ...
                key_o = list(self.o_micro_clusters.keys())[-1] + 1
                
                self.o_micro_clusters[key_o] = mc_from_p
                
                p = self.df_mc_to_points[(self.df_mc_to_points['x'] == point[0]) & (self.df_mc_to_points['y'] == point[1])]
                self.df_mc_to_points.loc[p.index[0], 'id_mc'] = (-1) * key_o
        ...
        if not merged_status and len(self.o_micro_clusters) == 0:
            ...
            p = self.df_mc_to_points[(self.df_mc_to_points['x'] == point[0]) & (self.df_mc_to_points['y'] == point[1])]
            self.df_mc_to_points.loc[p.index[0], 'id_mc'] = -2
    ...
    def _build(self):
        ...
        for minpts in range(self.m_minPoints, 1, -2):
            ...
            #print("> Time Total MinPts: ", end_time_minpts - start_time_minpts)
            
            # self.df_runtime.at[i, 'minpts']         = i
            # self.df_runtime.at[i, 'micro_clusters'] = len(self.p_micro_clusters)
            # self.df_runtime.at[i, 'mrg']            = (end_mrg - start_mrg)
            # self.df_runtime.at[i, 'mst']            = (end_mst - start_mst)
    ...
    def plot(self, matrix_partitions, count_DB, vertices, minpts, df_partition):
    ...
    def save_runtime(self):
        ...
        try:
            ...
            with open(os.path.join(m_directory, "runtime_t" + str(self.timestamp) + ".csv"), 'w') as writer:
                writer.write("minpts,micro_clusters,mrg,mst,dendrogram,selection,total\n")

                for _, linha in self.df_runtime.iterrows():
                    writer.write(str(linha['minpts']) + ',' + str(linha['micro_clusters']) + ',' + str(linha['mrg']) + ',' + str(linha['mst']) + ',' + str(linha['dendrogram']) + ',' + str(linha['selection']) + ',' + str(linha['total']) + "\n")
    ...
    def micro_clusters_to_points(self, timestamp):
        ...
        self.df_mc_to_points[(self.df_mc_to_points['id_mc'] != -1)].to_csv('results/datasets/data_t' + str(self.timestamp) + '.csv', index=False)
        
        df_plot = self.df_mc_to_points[(self.df_mc_to_points['id_mc'] != -1)]

        cmap = plt.get_cmap('tab10', len(list(set([row['id_mc'] for i, row in df_plot.iterrows()]))))

        plt.title("Timestamp: " + str(self.timestamp) + " | # Points: " + str(df_plot.shape[0]), fontsize=20)
